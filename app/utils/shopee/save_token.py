...
def save_token_to_db(access_token: str, refresh_token: str, expiry_time: datetime):
    db = SessionLocal()
    try:
        existing_token = db.query(TokenShopee).filter(TokenShopee.access_token == access_token).first()

        if existing_token:
            existing_token.refresh_token = refresh_token
            existing_token.expiry_time = expiry_time
            print(f"TokenShopee updated: {access_token}")
        else:
            new_token = TokenShopee(
                access_token=access_token,
                refresh_token=refresh_token,
                expiry_time=expiry_time,
            )
            db.add(new_token)
            print(f"TokenShopee saved: {access_token}")
